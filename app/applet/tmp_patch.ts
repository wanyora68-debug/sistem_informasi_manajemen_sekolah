...
if (idxStart !== -1 && idxEnd !== -1 && idxStart < idxEnd) {
    const replacement = `function unduhRekapDetailExcel() {
      ...
      function unduhRekapDetailPDF() {
         ...
         const chosenGuruName = document.getElementById('selGuruRekapSiswa') ? (document.getElementById('selGuruRekapSiswa') as HTMLSelectElement).value : 'ALL';
...
`;
    ...
    console.log('Successfully patched index.html with tsx & tmp_patch.ts!');
} else {
